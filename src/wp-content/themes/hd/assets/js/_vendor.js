...
class part {
  ...
  append(compo2) {
    ...
    return !!el.appendChild(compo2[ns]);
  }
  /**
  ...
  prepend(compo2) {
    const ns = this.ns, el = this[ns];
    return !!el.insertBefore(compo2[ns], el.firstElementChild || null);
  }
  /**
  ...
  remove(compo2) {
    const ns = this.ns, el = this[ns];
    return !!el.removeChild(compo2[ns]);
  }
  /**
...
class Data {
  ...
  compo(tag, name, props, defer = false, load = false, unload = false) {
    ...
    let compo2;
    if (defer) {
      compo2 = { ns, tag, name, props, load, unload };
    } else {
      compo2 = new Compo(ns, tag, name, props);
    }
    if (load && typeof load == "function") {
      compo2.load = props.onload = load;
    }
    if (unload && typeof unload == "function") {
      compo2.unload = props.onunload = unload;
    }
    return compo2;
  }
  /**
...
class SocialShare extends base {
  ...
  populate() {
    ...
    for (const intent of this.intents) {
      if (!intent in opts.scaffold)
        continue;
      const name = intent in locale2 ? locale2[intent].toString() : intent.replace(/\w/, (cap) => cap.toUpperCase());
      let title;
      switch (opts.scaffold[intent]) {
        case act.share:
          title = locale2.share.toString().replace("%s", name);
          break;
        case act.send:
          title = locale2.send.toString().replace("%s", name);
          break;
        case act.email:
          title = locale2.email.toString();
          break;
        case act.copy:
          title = locale2.copy.toString();
          break;
        case act.webapi:
          if (!navigator.share)
            continue;
          title = locale2["web-share"].toString();
          break;
      }
  ...
  intent(evt, target) {
    ...
    data.text = locale2.text.toString().replace("%s", data.text);
    if (intent in opts.scaffold) {
      switch (opts.scaffold[intent]) {
        case act.email:
  ...
  sendEmail(evt, data) {
    const { options: opts, encoder } = this;
    const locale2 = opts.locale;
    const url = opts.uriform["send-email"].replace("%subject%", encoder(locale2.subject)).replace("%text%", this.text(data));
  ...
  copyLink(evt, data) {
    ...
    if (opts.effects) {
      ...
      const msg = this.compo("span", "copied-link-msg", { innerText: locale2.copied });
